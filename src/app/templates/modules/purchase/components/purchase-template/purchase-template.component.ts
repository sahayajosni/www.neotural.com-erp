...
export class PurchaseTemplateComponent implements OnInit {
  ...
  getTemplateDetails(){
    this.model.templateType = "Purchase Invoice";
    this.purchaseservice.getTemplateDetails(this.model.templateType).subscribe(
      (data) => {
        if(data == null){
          this.btnlabel = "Save";
        }else{
  ...
  addTemplateDetails(){
    ...
    this.purchaseservice.addTemplateDetails(this.model).subscribe(
      (respose) => {
